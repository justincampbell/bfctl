...
**Safety caveat for `bfctl msp` scans.** Default scan range is 1..`SafeScanCeiling` (99). Going higher requires explicit `--max`. Reason: a full 1..199 scan against a LIONBEE_V1 (Betaflight 2025.12.1, AT32F435G) bricked the FC — firmware hung pre-USB-init, no LEDs, no enumeration, recoverable only by holding BOOT to enter Artery DFU and reflashing. The exact offending code was never bisected (would have required repeating the brick). Codes 1..130 were observed safe in the recovery sequence; the dangerous code(s) live in 131..199. Until somebody narrows it down, treat anything past 130 as "destructive on AT32 firmware" and don't default-include it. Specific named codes (e.g. `bfctl msp 188`) are still allowed — opt-in only.
