...
**Safety caveat for `bfctl msp` scans.** Default scan range is 1..`SafeScanCeiling` (199), with a `Denylist` (in `internal/msp/msp.go`) of MSP "in" handlers that, called with a 0-byte payload (which is what a scan sends), would reboot the FC, wipe storage, or corrupt config. Three categories:

- **Brick** — MSP_SET_OSD_CANVAS (188). A 1..199 scan against a LIONBEE_V1 (Betaflight 2025.12.1, AT32F435G) bricked the FC: firmware hung pre-USB-init, recoverable only by Artery DFU + reflash. Source review of `betaflight/src/main/msp/msp.c:4702` pinned it: with a 0-byte payload, sbufReadU8 reads past the buffer end (no bounds check, `streambuf.c:98`) into `canvas_cols`/`canvas_rows`, force-sets `video_system = HD` and `displayPortDevice = MSP`, then calls `writeEEPROM()` + `systemReset()` — persisting garbage OSD config to flash.
- **Scan-desync** — MSP_REBOOT (68). With a 0-byte payload, the FC defaults rebootMode to MSP_REBOOT_FIRMWARE, sends a 1-byte reply (the rebootMode byte), then registers `mspRebootFn` which calls `motorShutdown()` + `systemReset()`. The reboot happens *after* the reply lands, so a scan loop sees one normal-looking response at 68 and then total silence — every subsequent code times out because the FC is mid-reboot. Symptom: scan returned ~40 codes max despite the FC supporting ~104.
- **Silent destructive** — MSP_DATAFLASH_ERASE (72) wipes blackbox flash unconditionally; the other writers (141, 181, 183, 185, 186) corrupt running config in RAM. Denylisted defensively — a scan can't read writers anyway.

Single-code queries (e.g. `bfctl msp 188`) bypass the denylist for opt-in research and still respect MaxScanCode (199).
